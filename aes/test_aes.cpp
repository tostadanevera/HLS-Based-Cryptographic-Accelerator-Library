...
std::array<u8, AES_STATE_SIZE> hex_to_bytes(const std::string& hex)
{
    std::array<u8, AES_STATE_SIZE> out{};
    for (int i = 0; i < AES_STATE_SIZE; i++)
        out[i] = static_cast<u8>(std::stoi(hex.substr(i * 2, 2), nullptr, 16));
    return out;
}

std::string bytes_to_hex(const std::array<u8, AES_STATE_SIZE>& b)
{
    std::ostringstream oss;
    for (u8 byte : b) oss << std::hex << std::setw(2) << std::setfill('0') << static_cast<int>(byte);
    return oss.str();
}
...
int main()
{
    ...
    std::cout << "AES-128 tests\n";

    for (const auto& v : test_vectors)
    {
        total++;
        std::string key_hex(v.key), plain_hex(v.plain), expected_hex(v.cipher);

        if (key_hex.size() < 32 || plain_hex.size() < 32 || expected_hex.size() < 32)
            continue;

        std::array<u8, AES_KEY_SIZE> key = hex_to_bytes(key_hex);
        std::array<u8, AES_STATE_SIZE> plain = hex_to_bytes(plain_hex);
        std::array<u8, AES_STATE_SIZE> expected = hex_to_bytes(expected_hex);
        std::array<u8, AES_STATE_SIZE> cipher{}, decrypted{};
